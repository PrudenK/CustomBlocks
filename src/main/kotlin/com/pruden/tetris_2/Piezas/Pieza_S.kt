...
class Pieza_S (override var fila: Int, override var columna: Int,
    ...
    private val CENTRO = 68
    private val NUMPIEZA = 6
    private val COLOR = Color.LIMEGREEN

    private val FORMAS_S = arrayOf(
        arrayOf(
    ...
    companion object {
        const val NUMPIEZA_S = 6
        const val CENTRO_S = 68
        val COLOR_S = Color.LIMEGREEN!!
    }
        ...
        return if (nuevaOrientacion == 0) {
            if (columnaCentro != Globales.COLUMNAS - 1) {
                if (matrizNumerica[filaCentro - 1][columnaCentro] == BLANCO
                    ...
                    true
                } else condicionRotarEspecial(this,intArrayOf(0, -1),intArrayOf(-2, 0), true, -1)
            } else condicionRotarEspecial(this,intArrayOf(0, -1),intArrayOf(-2, 0), true, -1)
            ...
            if (filaCentro != Globales.FILAS - 1) {
                if (matrizNumerica[filaCentro][columnaCentro + 1] == BLANCO
                    && matrizNumerica[filaCentro + 1][columnaCentro + 1] == BLANCO) {
                    true
                } else condicionRotarEspecial(this,intArrayOf(-2, 0),intArrayOf(0, 1), false, -1)
            } else condicionRotarEspecial(this,intArrayOf(-2, 0),intArrayOf(0, 1), false, -1)
            ...
            if (columnaCentro != 0) {
                if (matrizNumerica[filaCentro + 1][columnaCentro] == BLANCO
                    && matrizNumerica[filaCentro + 1][columnaCentro - 1] == BLANCO) {
                    true
                } else condicionRotarEspecial(this,intArrayOf(1, 0),intArrayOf(0, 2), true, +1)
            } else condicionRotarEspecial(this,intArrayOf(1, 0),intArrayOf(0, 2), true, +1)
        } else {
            if (filaCentro != 0) {
                if (matrizNumerica[filaCentro][columnaCentro - 1] == BLANCO
                    && matrizNumerica[filaCentro - 1][columnaCentro - 1] == BLANCO) {
                    true
                } else condicionRotarEspecial(this,intArrayOf(0, 2),intArrayOf(-1, 0), false, +1)
            } else condicionRotarEspecial(this,intArrayOf(0, 2),intArrayOf(-1, 0), false, +1)
        }
    }

    override fun bajar(): Boolean {
        return bajar_3x2(this, intArrayOf(1, 1, 0, 2, 2, 1, 1, 2, 2, 1))
    }

    override fun derecha(): Boolean {
       return moverDerechaIzquierda_3x2(this, intArrayOf(1, 2, 2, 1, 1, 2, 2, 0, 1, 1), 1)
    }

    override fun izquierda(): Boolean {
       return moverDerechaIzquierda_3x2(this, intArrayOf(-2, -1, -1, -2, -1, -1, 0, -2, -2, -1), -1)
    }
    ...
    override fun clonar(): Piezas {
        return Pieza_S(this.fila, this.columna)
    }
}