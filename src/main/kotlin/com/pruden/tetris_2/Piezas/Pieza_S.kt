...
class Pieza_S (override var fila: Int, override var columna: Int) : Piezas(fila, columna) {
        ...
        return if (nuevaOrientacion == 0) {
            if (columnaCentro != COLUMNAS - 1) {
                if (matrizNumerica[filaCentro - 1][columnaCentro] == BLANCO
                    ...
                    true
                } else condicionRotar_especial(0, -2, -1, 0, true, -1)
            } else condicionRotar_especial(0, -2, -1, 0, true, -1)
            ...
            if (filaCentro != FILAS - 1) {
                if (matrizNumerica[filaCentro][columnaCentro + 1] == BLANCO
                    && matrizNumerica[filaCentro + 1][columnaCentro + 1] == BLANCO) {
                    true
                } else condicionRotar_especial( -2, 0, 0, +1, false, -1)
            } else condicionRotar_especial( -2, 0, 0, +1, false, -1)
            ...
            if (columnaCentro != 0) {
                if (matrizNumerica[filaCentro + 1][columnaCentro] == BLANCO
                    && matrizNumerica[filaCentro + 1][columnaCentro - 1] == BLANCO) {
                    true
                } else condicionRotar_especial( +1, 0, 0, +2, true, +1)
            } else condicionRotar_especial( +1, 0, 0, +2, true, +1)
        } else {
            if (filaCentro != 0) {
                if (matrizNumerica[filaCentro][columnaCentro - 1] == BLANCO
                    && matrizNumerica[filaCentro - 1][columnaCentro - 1] == BLANCO) {
                    true
                } else condicionRotar_especial( 0, -1, +2, 0, false, +1)
            } else condicionRotar_especial( 0, -1, +2, 0, false, +1)
        }
    }
