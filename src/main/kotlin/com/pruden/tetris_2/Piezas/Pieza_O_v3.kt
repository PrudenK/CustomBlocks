...
class Pieza_O_v3 (override var fila: Int, override var columna: Int) : Piezas(fila, columna) {
    ...
    private fun condicionRotar_especial(d_f1: Int, d_c1: Int, d_f2: Int, d_c2: Int, d_f3: Int, d_c3: Int, d_f4: Int, d_c4: Int, 
                                        d_f5: Int, d_c5: Int, columna_bolean: Boolean, movimiento: Int
    ): Boolean {
        return if (matrizNumerica[filaCentro + d_f1][columnaCentro + d_c1] == BLANCO 
            ...
            matrizNumerica[filaCentro + d_f5][columnaCentro + d_c5] == BLANCO) {
            limpiar()
            if (columna_bolean) {
                columna += movimiento
        ...
        return if (nuevaOrientacion == 0) {
            if (matrizNumerica[filaCentro - 1][columnaCentro - 1] == BLANCO
                ...
                if (matrizNumerica[filaCentro + 1][columnaCentro] != BLANCO
                    && matrizNumerica[filaCentro + 1][columnaCentro + 1] != BLANCO) {
                    condicionRotar_especial( -2, -1, -2, -1, -2, -1, -2, -1, -2, -1, false, -1)
                } else condicionRotar_especial( 1, 1, 1, 1, 1, 1, 0, 2, 1, 2, true, 1)
            }
            ...
            if (matrizNumerica[filaCentro - 1][columnaCentro + 1] == BLANCO
                ...
                if (matrizNumerica[filaCentro - 1][columnaCentro + 1] != BLANCO) {
                    condicionRotar_especial( -1, 0, 0, -1, 0, -2, 1, -1, 1, -2, true, -1)
                } else condicionRotar_especial( -1, 2, -1, 2, -1, 2, -1, 2, -1, 2, true, 1)
            }
            ...
            if (matrizNumerica[filaCentro - 1][columnaCentro - 1] == BLANCO
                ...
                if (matrizNumerica[filaCentro + 1][columnaCentro + 1] != BLANCO) {
                    condicionRotar_especial( 0, 1, -1, 0, -1, -1, -2, 0, -2, -1, false, -1)
                } else condicionRotar_especial( 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, false, 1)
            }
        } else {
            if (matrizNumerica[filaCentro - 1][columnaCentro + 1] == BLANCO
                && matrizNumerica[filaCentro][columnaCentro + 1] == BLANCO
                ...
                if (matrizNumerica[filaCentro][columnaCentro + 1] != BLANCO
                    || matrizNumerica[filaCentro - 1][columnaCentro + 1] != BLANCO) {
                    condicionRotar_especial( 1, -2, 1, -2, 1, -2, 1, -2, 1, -2, true, -1)
                } else condicionRotar_especial( 1, 0, 0, 1, 0, 2, -1, 1, -1, 2, true, 1)
            }
        }
    }
