...
class Pieza_Y_v2 (override var fila: Int, override var columna: Int,
    ...
    override fun puedeRotar(nuevaOrientacion: Int): Boolean { // faltan algunas rotaciones secundarias, no las veo muy necesarias ya veré si las añado
        ...
        return if (nuevaOrientacion == 0) {
            if (matrizNumerica[filaCentro + 1][columnaCentro - 1] == BLANCO 
                ...
                if (matrizNumerica[filaCentro + 1][columnaCentro - 1] != BLANCO) {
                    condicionRotarEspecial(this,intArrayOf(1, 0, 0, -1),intArrayOf(0, 1, 2, 1), true, 1)
                } else condicionRotarEspecial(this,intArrayOf(-1, -1, -1, 1),intArrayOf(-1, -1, -1, -2), true, -1)
            }
            ...
            if (matrizNumerica[filaCentro - 1][columnaCentro - 1] == BLANCO
                ...
                if (matrizNumerica[filaCentro + 1][columnaCentro] != BLANCO) {
                    condicionRotarEspecial(this,intArrayOf(-1, -1, -1, -2),intArrayOf(1, 1, 1, -1), false, -1)
                } else condicionRotarEspecial(this,intArrayOf(0, 1, 2, 1),intArrayOf(-1, 0, 0, 1), false, 1)
            }
            ...
            if (matrizNumerica[filaCentro][columnaCentro - 1] == BLANCO
                ...
                if (matrizNumerica[filaCentro][columnaCentro - 1] != BLANCO) {
                    condicionRotarEspecial(this,intArrayOf(1, 1, 1, -1),intArrayOf(1, 1, 1, 2), true, 1)
                } else condicionRotarEspecial(this,intArrayOf(-1, 0, 0, 1),intArrayOf(0, -1, -2, -1), true, -1)
            }
        } else {
            if (matrizNumerica[filaCentro - 1][columnaCentro] == BLANCO
                && matrizNumerica[filaCentro + 1][columnaCentro + 1] == BLANCO) {
                ...
                if (matrizNumerica[filaCentro + 1][columnaCentro + 1] != BLANCO) {
                    condicionRotarEspecial(this,intArrayOf(0, -1, -1, -2),intArrayOf(1, 0, -1, 0), false, -1)
                } else condicionRotarEspecial(this,intArrayOf(1, 1, 1, 2),intArrayOf(-1, -1, -1, 1), false, 1)
            }
        }
    }
    ...
    override fun derecha(): Boolean {
       return moverDerechaIzquierda_3x3(this, intArrayOf(1,1, 2,2, 0,0, 2,2, 1,1, 1,1, 0,0, 2,2, 1,1, 1,1, 1,1, 2,2), 1)
    }

    override fun izquierda(): Boolean {
       return moverDerechaIzquierda_3x3(this, intArrayOf(-1,-1, -1,-1, -2,-2, 0,0, -2,-2, -1,-1, -2,-2, -1,-1, -1,-1, -1,-1, -2,-2, 0,0), -1)
    }
    ...
    override fun clonar(): Piezas {
        return Pieza_Y_v2(this.fila, this.columna)
    }
}