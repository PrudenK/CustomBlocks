...
class Pieza_U (override var fila: Int, override var columna: Int) : Piezas(fila, columna) {
    private var orientacion = 0
    private var columnaCentro = columna+1
    private var filaCentro = fila

    private val CENTRO = 108
    private val NUMPIEZA = 10
    private val COLOR = Color.PINK

    private val FORMAS_U = arrayOf(
        arrayOf(
    ...
    companion object {
        const val NUMPIEZA_U = 10
        const val CENTRO_U = 108
        val COLOR_U = Color.PINK!!
    }
        ...
        return if (nuevaOrientacion == 0) {
            ...
            if (filaCentro != FILAS - 1) {
                if (matrizNumerica[filaCentro - 1][columnaCentro] == BLANCO
                    ...
                    true
                } else condicionRotar_especial( -2, 0, -1, 0, -2, +1, false, -1)
            } else condicionRotar_especial( -2, 0, -1, 0, -2, +1, false, -1)
            ...
            if (columnaCentro != 0) {
                if (matrizNumerica[filaCentro][columnaCentro - 1] == BLANCO
                    ...
                    && matrizNumerica[filaCentro + 1][columnaCentro - 1] == BLANCO) {
                    true
                } else condicionRotar_especial( 0, +1, 0, +2, +1, +2, true, +1)
            } else condicionRotar_especial( 0, +1, 0, +2, +1, +2, true, +1)
        } else {
            if (filaCentro != 0) {
                if (matrizNumerica[filaCentro - 1][columnaCentro] == BLANCO
                    ...
                    && matrizNumerica[filaCentro - 1][columnaCentro - 1] == BLANCO) {
                    true
                } else condicionRotar_especial( +2, -1, +2, 0, +1, 0, false, +1)
            } else condicionRotar_especial( +2, -1, +2, 0, +1, 0, false, +1)
        }
    }
    ...
    override fun bajar(): Boolean {
        return bajar3x3(this, intArrayOf(1, 1, 1, 1, 1, 1, 2, 2, 1, 1, 2, 2, -7, -7, 2, 2, 0, 2, 0, 2, 2, 2, -7, -7))
    }

    override fun derecha() {
        moverDerechaIzquierda_3x3(this, intArrayOf(0, 2, 2, 2, -7, -7, -7, -7, 2, 2, 2, 0, 2, 2, 1, 1, 2, 2, 1, 1, 1, 1, 1, 1), 1)
    }

    override fun izquierda() {
        moverDerechaIzquierda_3x3(this, intArrayOf(0, -2, -2, -2, -7, -7, -7, -7, -2, -2, -2, 0, -1, -1, -1, -1, -1, -1, -2, -2, -1, -1, -2, -2), -1)
    }
